...
// APIResponse is a response from the de-im API with the result
// stored raw.
...
// Error is an error containing extra information returned by the de-im API.
type Error struct {
	Code    int
	Message string
	ResponseParameters
...
// SentFrom returns the user who sent an update. Can be nil, if de-im did not provide information
...
type User struct {
	...
	// IsPremium true, if user has de-im Premium
	//
	// optional
...
type Message struct {
	...
	// PassportData is a de-im Passport data;
	//
	// optional
...
type Contact struct {
	...
	// optional
	LastName string `json:"last_name,omitempty"`
	// UserID contact's user identifier in de-im
	//
	// optional
...
// File contains information about a file to download from de-im.
type File struct {
	// FileID identifier for this file, which can be used to download or reuse
	// the file
	FileID string `json:"file_id"`
...
// ReplyKeyboardRemove Upon receiving a message with this object, de-im
...
type InlineKeyboardButton struct {
	...
	// used as a replacement for the de-im Login Widget
	//
	// optional
...
// de-im Login Widget when the user is coming from de-im. All the user
...
// ForceReply when receiving a message with this object, de-im clients will
...
type BotCommand struct {
	...
	Type    string          `json:"type"` // Command type, values can be: chat, user, message.
	Options []CommandOption `json:"options"`
}

type CommandOption struct {
	Name        string `json:"name"`
	Type        string `json:"type"` // Such as: string, integer, etc.
...
type BaseInputMedia struct {
	...
	// that exists on the de-im servers (recommended),
	// pass an HTTP URL for de-im to get a file from the Internet,
	...
	// (https://core.de-im.io/bots/api#formatting-options).
	//
	// optional
...
type WebhookInfo struct {
	...
	// happened when trying to synchronize available updates with de-im datacenters.
...
// InlineQuery is a Query from de-im for an inline request.
...
type LabeledPrice struct {
	...
	// (https://core.de-im.io/bots/payments/currencies.json),
...
type Invoice struct {
	...
	// (see https://core.de-im.io/bots/payments#supported-currencies)
	Currency string `json:"currency"`
...
type SuccessfulPayment struct {
	...
	// DeIMPaymentChargeID de_im payment identifier
	DeIMPaymentChargeID string `json:"de_im_payment_charge_id"`
